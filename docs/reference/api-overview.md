...
| `BillingConnector` | `connectToBilling(): SharedFlow<BillingConnectionResult>`. Transient `startConnection` failures are retried internally per `ConnectionRetryPolicy` before an `Error` is emitted — see [Error handling](../guides/error-handling.md#connection-setup-retry). Also `queryBillingAvailability(): BillingAvailability` — a deterministic `AVAILABLE` / `UNAVAILABLE` / `UNKNOWN` verdict that separates a *terminal* "this device can't pay" state from a *transient* failure, for safely gating irreversible decisions; defaulted to `UNKNOWN` on the interface. See [Deterministic availability](../guides/error-handling.md#deterministic-availability-for-irreversible-decisions). |
