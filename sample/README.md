...
`sample/build.gradle.kts` uses `implementation(project(":billing"))` rather than `implementation("com.kanetik.billing:billing:0.1.4")`. Library changes propagate to the sample on the next rebuild — no `publishToMavenLocal` step required during library development. Consumers in their own projects use the Maven coordinate; the sample is a development tool, not a consumer.
